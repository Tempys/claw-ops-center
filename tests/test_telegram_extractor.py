...
async def test_extract_node_extracts_github_url_and_fetches_readme():
    ...
    with patch(
        "news.nodes.telegram_extractor._fetch_readme", AsyncMock(return_value=readme)
    ):
        from news.nodes.telegram_extractor import telegram_extract_node

        result = await telegram_extract_node(
            {**STATE_BASE, "telegram_raw_signals": [_RAW_SIGNAL]}
        )
...
async def test_extract_node_drops_plain_text_signals():
    with patch(
        ...
        result = await telegram_extract_node(
            {**STATE_BASE, "telegram_raw_signals": [_PLAIN_SIGNAL]}
        )
...
async def test_extract_node_drops_signal_when_readme_fetch_fails():
    with patch(
        "news.nodes.telegram_extractor._fetch_readme", AsyncMock(return_value="")
    ):
        from news.nodes.telegram_extractor import telegram_extract_node
...
async def test_extract_node_strips_trailing_punctuation_from_repo_name():
    ...
    with patch(
        "news.nodes.telegram_extractor._fetch_readme",
...
async def test_extract_node_truncates_readme_to_1500_chars():
    ...
    with patch(
        "news.nodes.telegram_extractor._fetch_readme",
        AsyncMock(return_value=long_readme[:1500]),
    ):
        from news.nodes.telegram_extractor import telegram_extract_node
