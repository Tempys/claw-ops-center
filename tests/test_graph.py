...
async def test_graph_has_expected_nodes():
    with (
        patch("news.nodes.analyzer._client", AsyncMock()),
        patch("news.nodes.sender._bot", AsyncMock()),
    ):
...
async def test_full_graph_routes_filtered_signals_to_sender():
    ...
    mock_tg_pipeline = AsyncMock(
        return_value={"telegram_offset_id": 200, "filtered_signals": filtered}
    )
    ...
    with (
        patch("news.nodes.analyzer._client", AsyncMock()),
        patch("news.nodes.sender._bot", AsyncMock()),
        patch("news.graph.build_telegram_pipeline", return_value=mock_tg_pipeline),
        patch("news.graph.build_email_pipeline", return_value=mock_email_pipeline),
        patch("news.graph.sender_node", mock_send),
    ):
